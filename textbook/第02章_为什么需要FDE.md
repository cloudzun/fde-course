...
把这个公式画成曲线，就能一眼看清 FDE 与传统外包的分野——**同样是做项目，一个越做越省，一个越做越贵**（经验曲线示意：横轴为累计交付项目数 $n$，纵轴为单位交付成本；FDE 曲线按 $C_n=C_1 n^{-\alpha}$ 单调下降，外包曲线持平或上行）：

> [!NOTE]
> **（注：原"经验曲线图"文件缺失，此处以公式 + 下表替代图示；如需可视化，可按 $n^{-\alpha}$ 用任意图表工具绘制同一条曲线。）**

按此曲线，FDE 的成本随累计交付数 $n$ 按 $n^{-\alpha}$ 快速下坠（凸降曲线），而传统外包因缺乏平台复用、每单从头再来，成本始终持平甚至因复杂度上升而抬头。两条线之间不断张开的"剪刀差"，就是平台飞轮沉淀带来的**成本红利**——它随项目数增加而越拉越大。下面的数据表，正是这条曲线在几个关键节点上的具体展开：
