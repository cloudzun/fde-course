...
| **AIP Logic** | 编排大语言模型与业务逻辑 | 让 FDE 可以通过配置而不是深度编码，构建企业级 Agent。 |
...
>
> **（注：Palantir 官方文档的表述为"语义要素（对象/属性/关系）+ 动能要素（动作/函数/动态安全）"两大类（见附录 B-4）；本教材为便于一线理解，将其拆解为"数据/逻辑/操作"三要素，两者所指一致。）**
