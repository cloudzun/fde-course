...
- **异常终止（Abort）**：按合同约定的退出条款收尾——数据归还、系统移交、知识产权边界（见 15.6 从价值证据到合同条款的退出条款），并做一次失败复盘（参照 14.9 的口径），把教训回注到方法论。
...
> 1. **绘制**一个项目的利益相关者地图，标出决策者、影响者、执行者与反对者的立场（5.2）；
> 2. **辨别**"痛点/伪需求/快赢"，并用数据盘点表（SOP 2）排出数据可得性与质量清单（5.3–5.5）；
> 3. **选择**一个"快赢"场景，并用快赢矩阵（SOP 3）说明为什么是它（5.5）；
> 4. **评估**发现期的完成度，能逐条判断是否满足 Discovery Gate（5.7）；
...
> [!IMPORTANT]
> **双层 Gate：学习者能力 Gate × 真实项目 Gate**。真实项目 Gate（上面这些条目）需要**现场证据**——客户签字、生产指标、真实用户采用、SLA/MAU。对**课堂、自学或暂无真实客户**的读者，直接套真实 Gate 无法完成，故投入比例下设"学习者 Gate"，用**模拟材料**即可完成同一能力的验证（第6–9章的 Gate 同理，不再逐一重复）：
>
...
>
> 每项都要能答出：**证据是什么、谁负责、谁评审、最低通过标准、没有真实客户时的模拟替代证据**。学习者 Gate 通过只代表"能力达标"，不等于"项目可上线"。**这套双层规则也适用于第6/7/8章的各阶段 Gate。**
