...
**操作意义（给一线）：** 技术执行工程师（对标 Palantir Delta/FDE）需要从"黑客"转变为"架构师"和"布道师"。你需要面对脏数据、复杂的安全权限，以及无数个想放弃的用户。
...
> [!NOTE]
> 合规不仅限于技术：等保 2.0、数据安全法、个人信息保护法（PIPL）等制度约束，详见第 16 章的政企合规讨论与合规检查清单。技术安全基线只是其中"落地"的一环，制度与流程层面同样不可省略。
...
> [!NOTE]
> Build 阶段的重点是**培养出单个或少数几名深度冠军用户**（让系统先有人真正用起来、用出价值）。至于把冠军"由点连成网、依靠同侪影响力自发推广"，那是下一阶段（Scale）的任务——详见第 8 章的冠军用户网络建设。切勿在系统尚未做实做稳时就急于铺网。
