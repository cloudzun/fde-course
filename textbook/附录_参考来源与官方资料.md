...
> [!NOTE]
> **本附录的性质**：第 13 章对四家中国云厂商模式的描述（如"铁三角""TAM 模式""OTSS→FDE 演进"等）属于**业界通行的分析归纳**，并非逐条引自某篇官方文档。因此本附录提供的是**相关产品的官方页面**与**权威二手来源**，供读者按图索骥、核对基本事实；凡教材中的"模式命名/对比结论"，均为本教材基于公开信息的提炼，非厂商官方定性。所列 URL 截至编写时点有效，可能随时间失效。
...
> **与第 16 章的口径一致性**：AI/LLM 相关各领域（大模型底座、Agent 编排、RAG/向量库、评测、AI 编程、算力部署）的推荐**完全对齐第 16 章 16.4（中国本土技术栈）**，细节请**详见第 16 章 16.4**；本表仅做横向汇总并补充数据集成、建模、原型、协作、代码管理、可视化等领域。
...
### E-7　AI/LLM 集成（AI & LLM，详见第 16 章 16.4）
...
| Gate | 阶段验收关卡 | 每个阶段末的"检查清单 + 决策点"，全部通过才能进入下一阶段 | 约第 5 章 5.7 |
...
| Agent | 智能体 | 能自主执行"感知—决策—行动"闭环并调用工具/LLM 的软件组件 | 约第 15 章（AIP Logic 见 3.1） |
| MCP（Model Context Protocol） | 模型上下文协议 | 连接 AI 模型与外部工具/数据源的开放协议，让 Agent 跨端调用统一标准（**注：本教材正文未展开该协议，此处仅作术语收录**） | 约第 15 章 |
...
| 信创（信息技术应用创新） | 信创 / 国产化 | 以国产软硬件（OS/CPU/GPU/平台）替代国外产品以满足数据与供应链安全的工程体系 | 约第 5 章（深化见 16.4.5/16.4.6） |
| 等保（MLPS，等级保护） | 等级保护制度 | 我国对网络系统按重要性分等级、实施相应安全保护要求的制度（如等保 2.0） | 约第 4 章 4.3（详 14.5） |
| PIPL（个人信息保护法） | 个人信息保护法 | 我国规范个人信息收集、处理与保护的法律；处理敏感个人信息、向第三方提供或出境需单独同意 | 约第 14 章 14.5（亦见第 7 章） |
...
> [!NOTE]
> **使用说明**：本附录配合各章训练目标生成，每题含**题目 + 参考答案要点（2-4 行）**。题目刻意"可落地"——要求读者以**自己所在公司/项目**为对象分析、用**SOP 模板**实操、回顾**自身代码/经验**找复用模式，而非空谈概念。第 15–16 章（AI 篇）题目分别结合第 15 章（Echo 视角·甲方共识）与第 16 章（Delta 视角·技术与场景）作答。供讲师课堂研讨或学员自修使用。
...
### 第 15 章 AI FDE 落地（Echo 视角）：甲方共识与业务对齐

> [!NOTE]
> 以下题目结合第 15 章（15.1 全景、15.2 流程共识、15.3 技术选型共识、15.4 成熟度、15.5 甲方自评、15.6 合同协作、15.7 ROI）作答。

1. **用"AI 是放大器而非替代者"（15.1）这一判断，分析你团队里最值得用 AI 提效的一个环节，并指出"AI 能做什么、人必须把住什么"。**
   > 要点：选择"最可标准化、最重复"的环节（转写/写码/文档速读）；AI 负责提速，人负责判断业务真伪与质量把关；守住第 16 章 16.1 四条红线（尤其人工确认环）。
...
### 第 16 章 AI FDE 落地（Delta 视角）：技术与场景执行

> [!NOTE]
> 以下题目结合第 16 章（16.1 红线、16.2 AI 辅助四阶段、16.4 技术栈、16.5 场景地图、16.6 SOP 9、16.7 最佳实践、16.8 行业案例、16.9 反模式清单）作答。
...
3. **评估你的团队当前处于第 15 章 15.4 成熟度模型的哪一级，并列出提升到下一级的 3 个硬缺口（工具/流程/合规）及动作。**
   > 要点：按"个人提效 L1 → 流程嵌入 L2 → Agent 交付 L3 → AI 原生 L4"定位自己；重点识别"从 L2 升 L3"的最大跨越（需 Eval 体系 + 编排平台 + AI 作为交付物），把缺口写入团队 OKR（第 15 章 15.4）。
...
1. **用第 15 章 15.5 甲方自评清单判断：你的公司现在需要 FDE 吗？勾选并说明。**
...
2. **复盘你上一个项目的失败风险点，用第 14 章 14.9 的失败案例框架做一次根因分析。**
...
| 定制 | 我要的就是独一份，你们照做 | 定制会沉淀为可复用组件，反哺平台 | 甲方要"一次性独特性"，顾虑"把我们做成模板"。建议说明定制与"资产税/回注"机制的价值（见第 9 章、14.1），让甲方理解独特性也被保护。 |
...
| 政务 / 公共事业 | 自主可控是一票否决项——信创适配认证是硬指标 | 必须私有化部署，确保敏感数据不出内网 | 信创适配认证 + 内网闭环流转；存量系统年代久远无 API 时，可能需要视觉识别等额外操作能力 | 14.5.1、第 16 章 16.4.5 |
