...
| 能力回注（Feedback Loop / Upstreaming） | 能力回注 | 把现场的"一次性定制"抽象、沉淀为平台通用能力并反馈循环的机制 | 第9章（概念 1.3 起） |
...
2. **为团队每个角色写一条"最容易退化成外包"的行为红线，并给出纠偏动作。**
   > 要点：如技术执行只会写死代码不回注、业务策略变成纯客户经理、基础设施把环境外包化；对照各角色核心职责与"卖能力"基线（10.1-10.5）。
...
> [!NOTE]
> 以下 3 题结合第15章（15.1 全景、15.2 流程共识、15.3 技术选型共识、15.4 成熟度）作答。
...
> [!NOTE]
> 以下 3 题结合第16章（16.1 红线、16.2 AI 辅助四阶段、16.4 技术栈、16.5 场景地图、16.6 SOP 9、16.7 最佳实践）作答。
...
2. **用 SOP 9（16.6），为你的一个项目列出四个阶段的 AI 赋能动作、所用国产工具与风险等级，并给出风险控制。**
   > 要点：Discovery（转写/RAG 咨询，低）→ Prototype（AI 写码 + Agent Demo，中）→ Build（数据质量 AI 监控 + LLM 代码审查，中）→ Scale（自助 Copilot + Agent 运维，高）；高风险项必须人工确认环与回滚预案（16.1）。
